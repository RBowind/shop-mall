...
> 买家从购物车与收货地址提交订单：服务端定价、原子预占、幂等键防重复，下单成功时固化快照；下单产出待支付订单，支付确认见 `specs/order-payment/spec.md`。
...
- techspec: `docs/architecture/07-coupon-pay-lifecycle.md` §2、§4（下单预占与用券）
...
The system SHALL 在下单成功时固化商品与收货信息，历史订单不随商品、地址的后续变化。

#### Scenario: 商品后续变更不影响

- **WHEN** 订单创建后管理员改价、改名、下架该商品，或买家修改收货地址
- **THEN** 订单详情仍返回下单时的商品名、图、单价、数量与收货快照


### Requirement: 服务端定价下单

The system SHALL 以原子结果完成下单：预占库存与所选券、删除购物车行，产出待支付订单；积分余额不参与下单，支付确认见 `specs/order-payment/spec.md`。

#### Scenario: 下单成功

- **WHEN** 已登录买家 `POST /api/v1/orders`（Header 携带 Idempotency-Key，即幂等键，客户端生成的防重标识）提交购物车行、地址，可选携带 `coupon_id`
- **THEN** 下单成功时同时完成：创建 `pending_payment` 订单与明细快照；订单价=明细快照价合计（原价口径）、券抵扣金额、实付=原价-抵扣三值固化；固化支付截止时间；每个商品可售库存按数量预占（可售减、`hold_stock` 增）；所选券置 `held` 并绑定本订单；删除所选购物车行
...
- **THEN** 返回 2002 业务错误码（券不可用），订单、库存预占、券、购物车全部不变
...
- **WHEN** 某行数量大于商品当前可售库存
- **THEN** 请求失败并返回 2002 业务错误码（库存不足），整体回滚：无订单、无可售扣减与预占增加、券不预占、购物车不变

#### Scenario: 提交含已下架商品的行

- **WHEN** 提交的购物车行对应商品状态为 off_sale
- **THEN** 请求失败并返回 2002 业务错误码（商品下架），整体回滚
...
- **THEN** 只创建一个订单；另一请求以重放（返回原订单）或 409 结束；库存只预占一次、券至多预占一次，积分与流水不动
...
- 下单请求体的字段级定义（校验规则、错误码到端点的映射）以 `docs/api/openapi.yaml` 的 `OrderCreateRequest` 为准；券与支付端点的字段级契约尚未冻结进该文件。
