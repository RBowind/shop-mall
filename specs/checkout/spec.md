...
> 买家从购物车与收货地址提交订单：服务端定价、原子预占、幂等键防重复，成交瞬间固化快照；下单产出待支付订单，支付确认见 `specs/order-payment/spec.md`。
...
- **THEN** 下单成功时同时完成：创建 `pending_payment` 订单与明细快照；订单价=明细快照价合计（原价口径）、券抵扣金额、实付=原价-抵扣三值固化；固化支付截止时间；每个商品可售库存按数量预占（可售减、锁定增）；所选券置 `locked` 并绑定本订单；删除所选购物车行
