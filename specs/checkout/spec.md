...
The system SHALL 以原子结果完成下单：预占库存、锁定所选券、删除购物车行，产出待支付订单；积分余额不参与下单，支付确认见 `specs/order-payment/spec.md`。

#### Scenario: 下单成功

- **WHEN** 已登录买家 `POST /api/v1/orders`（Header 携带 Idempotency-Key，即幂等键，客户端生成的防重标识）提交购物车行、地址，可选携带 `coupon_id`
- **THEN** 下单成功时同时完成：创建 `pending_payment` 订单与明细快照；订单价=明细快照价合计（原价口径）、券抵扣金额、实付=原价-抵扣三值固化；固化支付截止时间；每个商品可售库存按数量预占（可售减、锁定增）；所选券置 `held` 并绑定本订单；删除所选购物车行
...
- **WHEN** 某行数量大于商品当前可售库存
- **THEN** 请求失败并返回 2002 业务错误码（库存不足），整体回滚：无订单、无可售扣减与锁定增加、券不锁定、购物车不变
