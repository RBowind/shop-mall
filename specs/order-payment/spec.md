...
> 下单产出待支付订单并预占库存与券；买家在支付时限内确认支付扣积分成交；超时未付由系统自动取消并释放占用。
...
The system SHALL 只允许买家本人把待支付（`pending_payment`）订单按实付积分一次性迁移为已支付（`paid`），订单状态、积分、流水、库存与优惠券状态必须全部成功变化，或全部不变化。
...
- **WHEN** 余额小于订单实付积分
- **THEN** 返回 2002 业务错误码（余额不足）；订单保持 `pending_payment`，库存与券的预占不变，余额、流水均不动；管理员补分后同一订单可再次支付成功
