...
> 当前目标状态：优惠券、待支付订单、确认支付、超时取消及七状态订单机以 [`../architecture/07-coupon-pay-lifecycle.md`](../architecture/07-coupon-pay-lifecycle.md) 为准；本文档集中的旧版“下单即支付”描述视为该 feature 的历史基线，不再作为行为契约输入。

## 1. 背景与目标

shop-mall 是一个积分商城：买家在微信小程序用积分领券、下单、确认支付、确认收货、申请退款，运营管理员在后台上下架商品、管理券模板、发货、审批退款、调整积分。后端是 Go 单体，价格计算、库存预占与扣减、积分账本、订单状态机和权限校验全部在服务端完成，两端前端只展示后端返回的结果。
...
范围限制：本期不接真实资金支付（确认支付只扣积分，接口形状预留），只做一单一张满减券，不做营销叠加；不采集物流单号、不做订阅消息与自动确认收货、不做验证码与忘记密码。边界外能力必须经变更提案进入，不顺手加入主链路。
...
- 买家进入领券中心领取满减券；选定地址与购物车项后下单，服务端按最新价格预占库存与券并生成待支付订单；买家确认支付时再扣积分并生成已支付订单。
- 买家查看自己的订单列表与详情，对已发货订单确认收货，对已支付订单申请退款；待支付订单超时后由系统自动取消。
- 买家查询自己的积分流水。
...
- 超级管理员创建、停发和查看券模板，审批退款，调整买家积分；积分调整携带幂等键并强制填写备注。
...
系统基础模型持久化 12 张表：买家域 `users`、`user_addresses`、`cart_items`，商品域 `products`，交易域 `orders`、`order_items`，账务域 `points_ledger`、`audit_logs`，后台权限域 `admin_users`、`roles`、`permissions`、`role_permissions`。优惠券与支付生命周期新增的 `coupon_templates`、`user_coupons` 及订单、库存变更见 [`../architecture/07-coupon-pay-lifecycle.md`](../architecture/07-coupon-pay-lifecycle.md)。完整基础字段、类型、CHECK 约束、索引、写入语义与删除策略见 [`data-model.md`](./data-model.md)。
...
erDiagram
    ...
    ORDER {
        ...
        string status "pending_payment | paid | shipped | completed | refund_requested | refunded | cancelled"
        bigint total_points
    }
    ORDER_ITEM {
        bigint id PK
...
系统主链路从买家浏览、领券、下单到订单终态（完成、退款或取消）运行，管理端的发货与退款审批嵌入订单生命周期。优惠券与支付的详细状态机见 feature techspec。鸟瞰：
...
flowchart LR
    ...
    B2 --> B3["加购物车"]
    B3 --> B4["下单"]
    B4 --> H["订单 pending_payment"]
    H -->|确认支付：扣积分| P["订单 paid"]
    H -->|超时取消：释放占用| X["订单 cancelled"]
...
当前订单状态机：`[*] → pending_payment → paid → shipped → completed`；`pending_payment → cancelled`；`paid → refund_requested → refunded`；`refund_requested → paid`（驳回）。状态迁移按允许路径校验；不存在买家主动取消和自动确认收货入口。

各主流程从触发到可观察结果的详细时序、幂等重放与失败处理，见 [`flows.md`](./flows.md)；优惠券、确认支付与超时取消的当前目标流程见 [`../architecture/07-coupon-pay-lifecycle.md`](../architecture/07-coupon-pay-lifecycle.md)。流程中的积分、库存、订单和券副作用必须原子生效，外部网络调用（微信 code2Session）不进入业务副作用。异步任务、周期任务与历史数据回填的口径以对应 feature techspec 和 `flows.md` 为准。
